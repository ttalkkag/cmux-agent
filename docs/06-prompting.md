...
- Prompt는 **전달 시점**에 메시지에 포함 — AI CLI 시작 시점이 아님
...
```json
{
  ...
  "instructions": "작업 완료 후 .agent/outbox/ 에 result artifact를 생성하세요.",
  "artifact_format": {
    "type": "result",
    "sender": "worker-1",
...
```python
def build_initial_orchestrator(workers: list[Agent], outbox_path: str) -> dict:
    """orchestrator 초기 안내 메시지를 생성한다."""

def build_initial_worker(name: str, inbox_path: str, outbox_path: str) -> dict:
    """worker 초기 안내 메시지를 생성한다."""

def build_dispatch_delivery(
    sender: str, task: str, outbox_path: str
) -> dict:
    """dispatch를 worker inbox에 전달할 메시지를 생성한다."""

def build_result_delivery(
    sender: str, result: str, workers: list[Agent], outbox_path: str
) -> dict:
    """result를 orchestrator inbox에 전달할 메시지를 생성한다."""
```
...
3. initial orchestrator 메시지 생성
4. initial worker 메시지 생성
5. artifact 형식 템플릿
6. 단위 테스트

## 설계 참고

- Prompt는 JSON 형식 — AI CLI가 파싱하기 쉽고, 사람도 읽을 수 있음
- artifact 형식 예시를 매번 포함 — AI CLI가 올바른 형식으로 응답할 확률을 높임
- worker 목록은 orchestrator에만 제공 — worker는 다른 worker를 알 필요 없음
- 언어는 한국어 기본, 영어 전환은 확장 예정
