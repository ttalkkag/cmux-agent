...
위 작업을 수행하세요.
완료 후 .agent/outbox/ 에 result artifact를 생성하세요.
```
...
추가 작업이 필요하면 .agent/outbox/ 에 dispatch artifact를 생성하세요.
